...
public class ValidateDebtorTransactionInFlex {
	...
	public void validateDebtorTransactionInFlexSingleTxn(String strDebtorAccountNumber, String strCurrentEndToEndId,
			String DebtorControlSum) throws IOException {
			...
			try {
				do {
					...
					if (dbDriver.contains("postgresql")) {
						...
						strDebtorTxnRecord = DatabaseUtil.databaseHandlerForFlexDB(strQueryGetDebtorTxnRecord,
						...
						strQueryGetDebtorTxnRecord = "SELECT DAT_TXN,DAT_VALUE,TXT_TXN_DESC,TXT_TXN_USER_DESC,REF_CHQ_NO,COD_DRCR,COD_CC_BRN_TXN,AMT_TXN,COD_TXN_CCY,REF_TXN_NO,REF_USR_NO "
								+ "FROM Ch_nobook " + "WHERE Cod_acct_no = '" + strDebtorAccountNumber + "' "
					...
					if (strDebtorTxnRecord != null) {
						...
						if (dbAmount == dbDebtorControlSum) {
							...
							/** INFO */
							System.out.println("Status=PASS: Debtor Transaction Record from Flex DB below");
							System.out.println("===========================================================");
							System.out.println();
							...
							System.out.println(strTxn_Date + ",	" + strValue_Data + ", " + strDebtorAccountNumber
						...
						System.out.println("Warning:: No Records were Found In The Database on the 1st attempt ,the script will attempt to retry in 20 seconds");
						Thread.sleep(20000);
						i++;
					}
